...
class UpdateBook extends JFrame implements ActionListener
{
	JPanel p,ph,p1,p2;
	ImageIcon img;
	JLabel  limg,l,l1,l2,lh,Bk_id,Bk_Nm,Bk_P_Nm,Bk_P_Y,Bk_Price,Bk_Q,Bk_pg,Bk_C,Bk_S;;
	JTextField t1,t2,t3,t4,t5,t6,t7,ts;
	Font f,f1,f2;
	JComboBox c1;
	JButton b1,b2,b3,b4;
	int count=0;
...
JOptionPane.showMessageDialog(null,"Enter Book publisher Name,It field should not be blank!! ");
			...
			else if(valid.isValidNumber(t3.getText()))
...
JOptionPane.showMessageDialog(null,"Enter Book Shelf,It field should not be blank!! ");
			...
			else
			{
				try
				{
						...
						int m;
m=JOptionPane.showConfirmDialog(this,"Are You Sure You Want to Update Book Record?","Confirm Message",JOptionPane.YES_NO_OPTION);
		if(m==0)
		{
		...
		ps1.setString(7,str);
		ps1.setString(8,t7.getText());
		ps1.setString(9,ts.getText());
		
				
		ps1.executeUpdate();
		JOptionPane.showMessageDialog(this,ts.getText()+"Record Successfully Updated in Database");
